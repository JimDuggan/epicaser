...
```{r,echo=T}
cases <- generate_epi_cases(Poisson = FALSE,RF = .3)
cases
```
...
We can plot the admissions and discharge data.

```{r}
ggplot()+
  geom_area(data=h_adm_sum,mapping=aes(x=DateAdmitted,y=Admissions,fill=CohortGroup))+
